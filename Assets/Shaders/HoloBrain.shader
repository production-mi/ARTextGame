Shader "Unlit/HoloBrain"
{
	Properties
	{
		...
		_GlowIntensity("Glow Intensity", Range(0.0, 1.0)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
			  ...
			  // ScanLines
				float scan = 0.0;
				scan = step(frac(i.worldVertex.y * _ScanTiling + _Time.y * _ScanSpeed), 0.5) * 0.65;
				...
				col = tex2D(_MainTex, i.uv) * _TintColor + (scan*0.5) + (glow * 0.35 * _TintColor) + rimColor;
				col.a = _Transparency - scan + (glow * _GlowIntensity)  +  0.1;
				return col;
			}
			ENDCG
		}
